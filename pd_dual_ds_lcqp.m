function sol = pd_dual_ds_lcqp(c, A, block_num, lambda, tol, max_iter, time_limit)

%%  doubble sweep ADMM
%%  Solver for following estimation Optimization Problem 
%%  min 1/2*x^T x + c^T x
%%  s.t.  A x = b
%%  Input (c, A, block_num, dual penalty parameter lambda, tolerence of ||Ax-b||_inf, max_iter, time_limit) 
...
%%  Use sparse chol (note: sometimes use non-sparse chol could be faster)
    ...
    %update primal
        ...
        % another sweep
        sub_model_prepare_time_start = tic;
        x_block_index_perm = block_num:-1:1;
        x_visited_block = 1;
        sub_model_prepare_time = toc(sub_model_prepare_time_start) + sub_model_prepare_time;
